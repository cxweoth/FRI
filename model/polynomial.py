...
class Polynomial:
    ...
    # can be replaced by FFT poly multiplication?
    ...
    def interpolate_domain(domain, values):
        # Interpolate
        ## P(x) = [(x-x2)(x-x3)...(x-xn)/(x1-x2)(x1-x3)...(x1-xn)]y1 + [(x-x1)(x-x3)...(x-xn)/(x2-x1)(x2-x3)...(x2-xn)]y2 + ... + [(x-x1)(x-x2)...(x-x{n-1})/(xn-x1)(xn-x2)...(xn-x{n-1})]yn
    ...
    def scale(self, factor):
        # P(cx) = a0 + c*a1x + c^2*a2x^2 + ... + c^n*anx^n
        return Polynomial([(factor^i) * self.coefficients[i] for i in range(len(self.coefficients))])

def test_colinearity(points):
    # univariate polynomial module anticipates a key operations in the FRI protocol, 
    # namely testing whether a triple of points fall on the same line – a fancy word for which is colinearity.
